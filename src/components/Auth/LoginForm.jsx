  ...
  const handleQuickLogin = async (account) => {
    try {
      ...
      if (errorMessage.includes('ECONNREFUSED') || errorMessage.includes('Failed to fetch')) {
        alert('Ошибка подключения к серверу. Убедитесь, что бэкенд запущен на порту 3001.');
      } else {
        alert(`Ошибка входа: ${errorMessage}\n\nУбедитесь, что тестовый аккаунт создан в базе данных.`);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        const data = await register(formData.name || formData.login, formData.login, formData.password);
        setUser(data.user);
      }
    } catch (error) {
