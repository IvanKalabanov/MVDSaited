...
const UserProfile = () => {
  ...
  // Имитация данных из базы (в реальном приложении это будет API запрос)
  const userRecords = [
    {
      id: 1,
  ...
  // Находим записи, связанные с текущим пользователем
  const userRecord = userRecords.find(record => 
    record.fullName.toLowerCase().includes(user.name.toLowerCase()) ||
    record.document.includes(user.login)
  );
  ...
  return (
    <div className="user-profile">
      ...
      <div className="profile-content">
        ...
        {user.role !== 'user' && (
          <div className="profile-section">
            ...
            <div className="info-grid">
              ...
              <div className="info-item">
                <label>Статус:</label>
                <span className={`status-indicator status-${employeeData.status}`}>
                  {employeeData.status}
                </span>
              </div>
            </div>
        ...
        {/* Информация из базы данных нарушителей */}
        <div className="profile-section">
          <h2>Информация из базы данных</h2>
          {userRecord ? (
            <div className="database-record">
              <div className="record-header">
                <h3>Запись в базе данных</h3>
                <span className={`case-badge case-${userRecord.caseType === 'Уголовное' ? 'criminal' : 'admin'}`}>
                  {userRecord.caseType}
                </span>
              </div>
              ...
              <div className="record-details">
                <div className="detail-row">
                  ...
                  <span>{userRecord.caseNumber}</span>
                </div>
                <div className="detail-row">
                  <label>Статус дела:</label>
                  <span className={`status-badge status-${userRecord.status.toLowerCase()}`}>
                    {userRecord.status}
                  </span>
                </div>
                <div className="detail-row">
                  <label>Ответственный сотрудник:</label>
                  <span>{userRecord.officer}</span>
                </div>
                <div className="detail-row">
                  <label>Подразделение:</label>
                  <span>{userRecord.department}</span>
                </div>
                <div className="detail-row">
                  <label>Документ:</label>
                  <span>{userRecord.document}</span>
                </div>
                <div className="detail-row full-width">
                  <label>Описание:</label>
                  <span>{userRecord.description}</span>
                </div>
              </div>
            </div>
          ) : (
            <div className="no-record">
              ...
              <p>В базе данных нарушителей нет записей, связанных с вами</p>
            </div>
          )}
        </div>
